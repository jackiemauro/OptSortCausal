...
get.psihatU.mu <- function(D){
  #only fluctuate mu
  dat = D$data; mumat = D$muhat; pimat = D$true.pimat; f.vec = D$muhat.min
  Avals = sort(unique(dat$a))
  temp = sapply(Avals, function(x) as.numeric(Avals[f.vec] == x))
  mu.hat = diag(mumat %*% t(temp))
  pi.hat = diag(pimat %*% t(temp))
...
get.psihatU.pi <- function(D){
  #only fluctuate pi
  dat = D$data; mumat = D$true.mumat; pimat = D$inv.pihat; f.vec = D$muhat.min
  Avals = sort(unique(dat$a))
  temp = sapply(Avals, function(x) as.numeric(Avals[f.vec] == x))
  mu.hat = diag(mumat %*% t(temp))
  pi.hat = diag(pimat %*% t(temp))
