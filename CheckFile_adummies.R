...
for(vfold in 1:nsplits){
  ...
  ifAm = (as.numeric(at.A[s!=vfold] == Avals[fAmhat])/pihatAm)*(test.df$y - muhatAm) + muhatAm
  psiAm = mean(ifAm)
...
write.csv(if.results, 'checkfileifresults.csv')

pi.results = cbind(apply(plugins,1,mean),apply(pi.sds,1,mean))
write.csv(pi.results, 'checkfilepiresults.csv')
