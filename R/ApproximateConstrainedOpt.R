...
approx.constr.opt.causal.nm <- function(df,aLevel,obsD,
  ...
  for(rnd in 1:2){
    ...
    ### outputs for matlab
    out.muF[s==rnd,] <- muF
    constr <- round(apply(aLevel[s==rnd,],2,sum) * 1.05)
    ...
    assig.vec[s==rnd] <- f.con
    est.assig.vec[s!=rnd] <- fhat
    ifs[s!=rnd] <- ifvals
  }
